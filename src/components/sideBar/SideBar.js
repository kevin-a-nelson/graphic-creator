...
class SideBar extends React.Component {
  ...
  async fetchEvent() {
    ...
    const event = await myAxios.get(url).then((response) => {
  ...
  async setNewTeams() {
    ...
    const division =
      this.state.selectedDivision.value || divisions[divisions.length - 1];
    ...
    if (pool === "All") {
      ...
      newTeamsIdx += 1;
      let teamNames = event.Divisions[this.state.selectedDivision.value].Pools[
        poolLetter
      ].map((team) => [`${team.Wins} - ${team.Loses}`, team.Name]);
  ...
  async onDownload(title, bracketNumber) {
    ...
    const teamNames = JSON.stringify(
      this.props.teams.text.map((text) =>
        text.name ? text.name.replace("#", "_HASHTAG") : ""
      )
    );
    ...
    console.log();
    const imageString = await axios
      .get(
        `https://graphic-creator-screenshoter.herokuapp.com/screenshots/${display}/${compressedTeamNames}`
      )
  ...
  render() {
    ...
    if (pools !== []) {
      poolOptions = pools.map((pool) => ({
        label: pool.charCodeAt(0) - 64,
        value: pool.charCodeAt(0) - 64,
      }));
      poolOptions.unshift({
        label: "All",
        value: "All",
      ...
      <div className="sidebar-container">
        <div className="sidebar">
          ...
          <button
            ...
            onClick={() =>
              this.onDownload(this.props.title, this.props.bracketNumber)
            }
          >
            Download
          </button>
